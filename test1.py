...
import numpy as np
import pandas as pd
from indicators import preprocess_and_generate_signals  # Indicator preprocessing
from model import PPOAgent
from data_fetcher import fetch_live_data  # Fetch live data function
...
def get_ppo_state(df):
    ...
    # Use the most recent state from the dataframe
    state = df[['Close', 'Volume', 'RSI', 'EMA_9', 'EMA_21', 'BB_Upper', 'BB_Lower']].values[-1]
    return state
...
def live_trading_loop(symbol, leverage, amount_in_usdt, model):
    ...
    try:
        ...
        while True:
            ...
            log_message(f"Current State Indicators: RSI={live_data['RSI'].values[-1]}, EMA_9={live_data['EMA_9'].values[-1]}, EMA_21={live_data['EMA_21'].values[-1]}, Price={current_price}")
            ...
            if not position_opened:
                ...
                if action == 0:  # Buy/Long
                    ...
                    stop_loss = set_stop_loss(entry_price, "BUY", stop_loss_percent=0.02)
...
if __name__ == "__main__":
    # Define your symbol, leverage, and amount for trading
    symbol = "BTCUSDT"  # Example trading pair
    leverage = 10  # Set leverage
    amount_in_usdt = 100  # Amount to trade with in USD

    # Initialize PPO agent (ensure your model is properly loaded)
    model = PPOAgent.load("ppo_trading_model")

    # Run the live trading loop
    live_trading(symbol, leverage, amount_in_usdt, model)
...
def calculate_reward(profit_or_loss, hit_stop_loss=False, transaction_cost=0.001):
    """Calculate reward considering profit, stop-loss, and transaction costs."""
    reward = profit_or_loss - transaction_cost  # Penalize for transaction cost
    if hit_stop_loss:
...
def train_ppo_agent():
    """Training PPO agent with market data and live experiences."""
    ...
    for episode in range(EPISODES):
        ...
        if data is None or data.empty:
            print("Skipping training: Data is empty or None.")
        ...
        for step in range(len(data) - 1):
            ...
            # Check state size before proceeding
            ...
            # Compute reward
            reward = calculate_reward(profit_or_loss=1.0)  # Example reward logic
            done = False  # Update logic if needed
...
def calculate_bollinger_bands(df, window=20, num_std=2):
    ...
    df['SMA'] = df['Close'].rolling(window=window).mean()  # Calculate Simple Moving Average
    rolling_std = df['Close'].rolling(window=window).std()
    df['BB_Upper'] = df['SMA'] + (rolling_std * num_std)  # Upper Band
    df['BB_Lower'] = df['SMA'] - (rolling_std * num_std)  # Lower Band

    # Fill any NaN values that might be generated in the first few rows
...
def calculate_macd(df):
    # Ensure that you are calculating MACD correctly
...
def calculate_indicators(df):
    """Calculate RSI, EMA, Bollinger Bands, and MACD."""
    df = calculate_rsi(df)  # Calculate RSI
    df = calculate_ema(df, window=21)  # Calculate EMA with window of 21
    df = calculate_ema(df, window=50)  # Calculate EMA with window of 50
    df = calculate_ema(df, window=200)  # Calculate EMA with window of 200
    df = calculate_bollinger_bands(df)  # Calculate Bollinger Bands
    df = calculate_macd(df)  # Calculate MACD
    
    return df

# Function to preprocess data, calculate indicators, and ensure proper column alignment
def preprocess_and_generate_signals(df):
    """Preprocess the data, calculate indicators, and ensure there are no NaN values."""
    
    df['Close'] = pd.to_numeric(df['Close'], errors='coerce')
    df['Volume'] = pd.to_numeric(df['Volume'], errors='coerce')
    
    # Forward fill missing values to avoid dropping rows
    df['Close'] = df['Close'].ffill()
    df['Volume'] = df['Volume'].ffill()

    if len(df) < 30:
        print("Not enough data to calculate indicators. Skipping processing.")
        return None
...
# Example: Modify combined_trading_signal to use RSI thresholds for stronger signals
...
# Example: Get a combined trading signal for 'BTCUSDT'
signal = combined_trading_signal(symbol='BTCUSDT')
...
# Initialize your PPO agent before starting live trading
def start_live_trading(symbol, model, scaler, agent, leverage=6, trade_amount=1, seq_length=60):
    balance = 20
    position = None
    total_profit = 0
    stop_loss = None
    ...
    while True:
        try:
            # Fetch live data
            live_data = fetch_live_data(symbol, seq_length=seq_length, scaler=scaler)

            if live_data is None:
                print(f"No valid live data for {symbol}. Retrying...")
                time.sleep(60)  # Wait before the next iteration
                continue

            # Agent acts based on live data
            action = agent.act(live_data)

            # Get the current market price from live data (not scaled)
            current_market_price = float(fetch_live_data(symbol, seq_length=1)[0][0])

            # Open position logic based on the action (1 for BUY, 2 for SELL)
            if position is None:
                if action == 1:  # BUY action
                    position = "BUY"
                    entry_price = current_market_price
                    print(f"Executing BUY at {current_market_price}")
                    execute_trade("BUY", trade_amount, leverage, symbol)
                    stop_loss = set_stop_loss(entry_price, side="BUY")
                    take_profit = set_take_profit(entry_price, side="BUY")

                elif action == 2:  # SELL action
                    position = "SELL"
                    entry_price = current_market_price
                    print(f"Executing SELL at {current_market_price}")
                    execute_trade("SELL", trade_amount, leverage, symbol)
                    stop_loss = set_stop_loss(entry_price, side="SELL")
                    take_profit = set_take_profit(entry_price, side="SELL")

            # Close position logic based on stop-loss/take-profit
            elif position == "BUY":
                if current_market_price <= stop_loss:
                    profit = (current_market_price - entry_price) * leverage
                    total_profit += profit
                    balance += profit
                    print(f"Closed BUY with profit {profit} at {current_market_price} (Stop-Loss Triggered)")
                    close_position()
                    position = None
                    ...
                    total_profit += profit
                    balance += profit
                    print(f"Closed BUY with profit {profit} at {current_market_price} (Take-Profit Triggered)")
                    close_position()
                    position = None
                ...
                if current_market_price >= stop_loss:
                    ...
                    total_profit += profit
                    balance += profit
                    print(f"Closed SELL with profit {profit} at {current_market_price} (Stop-Loss Triggered)")
                    close_position()
                    position = None
                elif current_market_price <= take_profit:
                    profit = (entry_price - current_market_price) * leverage
                    total_profit += profit
                    balance += profit
                    print(f"Closed SELL with profit {profit} at {current_market_price} (Take-Profit Triggered)")
                    close_position()
                    position = None

            # Log current balance and total profit
            print(f"Current Balance: {balance}, Total Profit: {total_profit}")

            # Sleep before fetching live data again
            time.sleep(60)
...
def fetch_live_data(symbol, seq_length=60):
    """
